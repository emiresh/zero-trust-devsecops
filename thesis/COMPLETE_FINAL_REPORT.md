---
title: "Automating Continuous Compliance in DevSecOps Through Zero Trust: A Kubernetes Microservices Reference Implementation"
author: "Iresh Emalsha"
programme: "Master of Information Systems (MIS)"
credits: 5
date: "April 2026"
---

# Automating Continuous Compliance in DevSecOps Through Zero Trust: A Kubernetes Microservices Reference Implementation
...
This project addresses that problem by designing and implementing a Zero-Trust DevSecOps framework for a Kubernetes-based microservices application called FreshBonds. The systematic approach uses the Design Science Research Methodology (DSRM) to construct and evaluate an artefact that combines GitOps-driven deployment, infrastructure as code, policy as code, container vulnerability scanning, automated secret rotation, runtime threat detection, and AI-assisted incident reporting. The framework is implemented as a production-deployed system running on a three-node Kubernetes cluster on Oracle Cloud Infrastructure (OCI), secured by 39 automated policy rules across Kyverno, Open Policy Agent, and Falco, six specialised GitHub Actions CI/CD pipelines integrating ten security tools, and an AI-augmented security monitoring layer that enriches runtime security events through Azure OpenAI GPT-4o-mini.

The major finding is that continuous compliance can be operationalised as a set of automated control points across the delivery lifecycle rather than as a periodic manual audit. Repository investigation confirms nine Kyverno rules, sixteen OPA checks, thirteen custom Falco runtime rules, twenty-two Prometheus alerting rules, six CI/CD workflows, dual-format Software Bill of Materials (SBOM) generation, monthly automated secret rotation with Git-based audit trails, and security scan artefact upload. Evaluation against NIST SP 800-207 Zero Trust Architecture principles demonstrates compliance across all seven core tenets, with the framework blocking one hundred per cent of deployments containing CRITICAL Common Vulnerabilities and Exposures (CVEs) and detecting runtime security events within thirty seconds through eBPF-based kernel instrumentation. The contribution is a reproducible MIS-level reference implementation that translates Zero Trust from a conceptual security model into concrete DevSecOps automation for Kubernetes.
...
This project is scoped as a 5-credit MIS final project. Table 1 defines the boundaries of what is included and excluded from the project scope.
...
Large Language Models (LLMs) such as GPT-4o-mini (Microsoft, 2024) offer the potential to reduce Mean Time to Understand (MTTU) by automatically generating contextual threat assessments, investigation steps, and recommended actions for security events. By enriching raw security alerts with natural language analysis, LLMs can help analysts prioritise high-severity incidents and understand the potential impact of detected threats without requiring deep expertise in the specific detection rule that triggered the alert.
...
![Figure 1: Six-layer Zero-Trust DevSecOps architecture overview](diagrams/figure-01-six-layer-architecture.png)
...
![Figure 2: Infrastructure topology on Oracle Cloud Infrastructure](diagrams/figure-02-oci-infrastructure.png)
...
- Legacy Instance Metadata Service (IMDS) endpoints disabled (`are_legacy_imds_endpoints_disabled = true`) to prevent SSRF-based credential theft.
- In-transit encryption enabled for block volumes (`is_pv_encryption_in_transit_enabled = true`).
...
![Figure 3: Kubernetes cluster namespace and component layout](diagrams/figure-03-kubernetes-namespaces.png)
...
![Figure 4: FreshBonds microservices application architecture](diagrams/figure-04-microservices-architecture.png)
...
| **Frontend** | React 18, Vite, TailwindCSS, Nginx (Alpine) | 80 | — | Multi-stage Docker build (node:18-alpine builder → nginx:alpine runtime), build-time environment configuration (VITE_API_URL), read-only root filesystem, non-root nginx user |
...
![Figure 5: CI/CD pipeline architecture with security gates](diagrams/figure-05-cicd-pipeline-architecture.png)
...
![Figure 6: GitOps deployment flow via Argo CD](diagrams/figure-06-gitops-deployment-flow.png)
...
![Figure 7: Policy-as-code enforcement points across the lifecycle](diagrams/figure-07-policy-enforcement-points.png)
...
![Figure 8: Secret lifecycle management and rotation workflow](diagrams/figure-08-secret-lifecycle.png)
...
![Figure 9: Runtime security event flow from Falco to AI enrichment](diagrams/figure-09-runtime-security-flow.png)
...
9. **Suspicious File Modification** (CRITICAL) — Detects writes to `/etc/passwd`, `/etc/shadow`, crontab, or `authorized_keys`. Maps to MITRE T1543 (Create or Modify System Process).
...
![Figure 10: Observability stack data flow](diagrams/figure-10-observability-stack.png)
...
![Figure 11: NIST SP 800-207 compliance mapping visualisation](diagrams/figure-11-nist-compliance.png)
...
![Figure 12: MITRE ATT&CK for Containers coverage heat map](diagrams/figure-12-mitre-attack-coverage.png)
...
| 11 | dangerous-capabilities | deny | Must not add `SYS_ADMIN`, `NET_ADMIN`, `ALL` capabilities |
...
|---|---|
| `sensitive_files` | `/etc/shadow`, `/etc/sudoers`, `/etc/pam.d/*`, `~/.ssh/*`, `~/.aws/*`, `~/.kube/*` |
